...
public class System_manage {
     ...
     public String getName(String email)
    {
        ...
        try
        {
            while(result.next())
            {
                fname =result.getString("Fname" +"Lname");
    ...
    public Branch Search_branch(int Branch_id) {
     ...
     public  ArrayList<Massage> getAllMessage1(int recieved_id)
     {
          DB_controller Db = DB_controller.Get_DB_controller();
        Db.Connect();
         try{
              ...
              HashMap <String , String> result2 = new HashMap();
               ...
               ArrayList <Massage> list = new ArrayList<>(100);
               Massage m = new Massage();
               int message_id ;
             ResultSet result = Db.Select(" * ", " recieved ", " Reciever_id = " + "'"+recieved_id+"'" + "and State_id = " + "'"+5+"'");
        try {
            while(result.next())
            {
              message_id = result.getInt("Message_id");
              m.setState(result.getInt("State_id"));
              ids.add(message_id);
            }
        } catch (SQLException ex) {
            Db.Close();
         ...
         System.out.println(ids);
            ...
            result2 = Db.SelectStatement(" * ", " message ", " Message_id = " + "'"+ids.get(i)+"'");
             if(result2 != null)
             {
               m.setId(Integer.parseInt(result2.get("Message_id")));
               m.setSenderId(Integer.parseInt(result2.get("sender_id")));
               m.setDate_id(Integer.parseInt(result2.get("Date_id")));
               m.setTime(result2.get("Time"));
               m.setContent(result2.get("Content"));
     ...
     public ArrayList<Massage> getAllMessage(int recieved_id) throws SQLException
     {
        DB_controller Db = DB_controller.Get_DB_controller();
        Db.Connect();
        Massage m = new Massage();
        ...
        try {
         ...
         {
         result3 = Db.Select(" * ", " message ", " Message_id = " + "'"+ids.get(i)+"'");
         
       
            while(result3.next())
            {
                m=new Massage();
               m.setId(result3.getInt("Message_id"));
               m.setSenderId(result3.getInt("sender_id"));
               m.setDate_id(result3.getInt("Date_id"));
               m.setTime(result3.getString("Time"));
               m.setContent(result3.getString("Content"));
        ...
        return list;
          ...
          public boolean ReplyMessage ( String senderid , String reciverid ,  String content ,String messageid) 
    {
         DB_controller DB = DB_controller.Get_DB_controller();
         DB.Connect();
          ...
          H.put("sender_id", senderid);
           ...
           Date date1 = new Date();
            String currentdate = dateFormat2.format(date1); 
          
           ResultSet result = DB.Select(" Date_id ", " date ", " Date = " + "'"+currentdate+"'" );
        
        try {
            if(result.next())
            {
                dateId = result.getInt("Date_id");
                H.put("Date_id", ""+dateId);
            }
            else{
                H3.put("Date", ""+currentdate);
                int id = DB.Insert("date", H3);
                H.put("Date_id", ""+id);
            }
        } catch (SQLException ex) {
            Logger.getLogger(System_manage.class.getName()).log(Level.SEVERE, null, ex);
           ...
           int message_id =  DB.Insert("message", H);
           H2.put("Message_id", ""+message_id);
           H2.put("Reciever_id", ""+reciverid);
           H2.put("State_id", ""+5);
           
           DB.Insert("recieved", H2);
           
    
           
           DB.Update("recieved", "State_id = 4", "Message_id = " + "'"+messageid+"'");
           
         return true;
    }
     
     public ArrayList<String> differentTime(int reciever_id) throws SQLException, ParseException
     {
         DB_controller Db = DB_controller.Get_DB_controller();
         Db.Connect();
         String time="";
         ...
         ArrayList <Date> list3 = new ArrayList<>(100);
         ArrayList <String> allDate = new ArrayList<>(100);
         
         list = this.getAllMessage(reciever_id);
           ...
           for(int j = 0; j < list2.size(); j++) {
            ...
            time = list2.get(j);    
             ...
             Timestamp timestamp1 = new Timestamp(date1.getTime());
                ...
                //System.out.println("time " + timestamp1);
                list3.add(timestamp1);
                 
               
    }
            for(int x = 0; x < list3.size(); x++) {
       
             Date convertedTime1 = list3.get(x); 
             Timestamp timestamp1 = new Timestamp(convertedTime1.getTime());
             //System.out.println("time " + timestamp1);
             DateFormat dateFormat = new SimpleDateFormat("HH:mm:ss");
             Date date = new Date();
             String time2 = dateFormat.format(date); 
             Date convertedTime2 = dateFormat.parse(time2);
             Timestamp timestamp2 = new Timestamp(convertedTime2.getTime());
    ...
    allDate.add(value);
               // System.out.println("timeeee" + value);    
    }
     ...
     public String getSenderMessage(int sender)
     {
        DB_controller Db = DB_controller.Get_DB_controller();
        Db.Connect();
        ResultSet result = Db.Select(" * ", " user ", " User_id = " + "'"+sender +"'");
      ...
      public String getDateMessage(int dateId)
     {
        DB_controller Db = DB_controller.Get_DB_controller();
        Db.Connect();
        ResultSet result = Db.Select(" * ", " date ", " Date_id = " + "'"+dateId +"'");
        ...
        public int getCountMessage(int receiver)
     {
        DB_controller Db = DB_controller.Get_DB_controller();
        Db.Connect();        int i=5;                                           
        int result = Db.SelectCount(" recieved ", " recieved_id ", " Reciever_id = " +"'"+receiver+"'" + "and"+" State_id = " + "'"+i+"'");
